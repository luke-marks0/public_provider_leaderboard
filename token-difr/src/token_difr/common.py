...
def _is_gpt_oss_harmony_model(tokenizer, model_name: str | None = None) -> bool:
    """Detect whether a tokenizer/model pair uses Harmony-formatted gpt-oss outputs."""
    candidates = []
    if model_name:
        candidates.append(str(model_name))
    tokenizer_name = getattr(tokenizer, "name_or_path", None)
    if tokenizer_name:
        candidates.append(str(tokenizer_name))
    combined = " ".join(candidates).lower()
    return "gpt-oss" in combined


def _build_harmony_assistant_response(content: str, reasoning: str | None) -> str:
    """Reconstruct a gpt-oss assistant completion in Harmony wire format.

    We intentionally omit trailing stop markers like <|return|> because upstream
    chat APIs do not surface them in text fields, and generation can be truncated
    before a stop marker when max_tokens is reached.
    """
...
def encode_thinking_response(
    ...
    *,
    ...
    """
    if _is_gpt_oss_harmony_model(tokenizer, model_name=model_name):
        full_response = _build_harmony_assistant_response(content, reasoning)
    elif reasoning:
...
def _serialize_message_content(content: Any) -> str:
    """Convert potentially structured chat content into a stable plain-text form."""
...
def _is_deepseek_v32_model(tokenizer, model_name: str | None = None) -> bool:
    """Detect DeepSeek-V3.2 models that require Python-side prompt encoding."""
    candidates = []
    if model_name:
        candidates.append(str(model_name))
    tokenizer_name = getattr(tokenizer, "name_or_path", None)
    if tokenizer_name:
        candidates.append(str(tokenizer_name))
    tokenizer_type = getattr(tokenizer.__class__, "__name__", "")
    if tokenizer_type:
        candidates.append(str(tokenizer_type))
    combined = " ".join(candidates).lower()
    return "deepseek-v3.2" in combined or "deepseek_v32" in combined


def _render_deepseek_v32_chat_prompt(
    conversation: list[dict[str, Any]],
    *,
    add_generation_prompt: bool = True,
) -> str:
    """Render DeepSeek-V3.2 chat prompts per encoding_dsv32.py (chat mode).
    ...
    rendered_messages: list[str] = [bos_token]
    last_idx = len(conversation) - 1

    for idx, message in enumerate(conversation):
        role = str(message.get("role", "user")).strip().lower() or "user"
        content = _serialize_message_content(message.get("content", ""))
        is_last_message = idx == last_idx
        append_assistant_prefix = add_generation_prompt or not is_last_message
        ...
        if role == "assistant":
            rendered_messages.append(f"{content}{eos_token}")
            continue
        ...
        if role in {"user", "developer"}:
            if append_assistant_prefix:
                rendered_messages.append(
                    user_msg_template.format(content=content) + thinking_end_token
                )
            else:
                rendered_messages.append(f"<｜User｜>{content}")
            continue

        # Unsupported roles (e.g. tool) are rendered deterministically rather than failing.
        rendered_messages.append(f"{role}: {content}")

    return "".join(rendered_messages)


def _render_fallback_chat_prompt(
    conversation: list[dict[str, Any]],
    *,
    add_generation_prompt: bool = True,
) -> str:
    """Render a conversation when the tokenizer has no chat template.

    This fallback is only used for tokenization/length filtering and produces a
    deterministic role-tagged plain-text transcript.
    """
...
def render_conversation_for_tokenization(
    tokenizer,
    conversation: list[dict[str, Any]],
    *,
    add_generation_prompt: bool = True,
    ...
    """Render conversation text for tokenizer encoding.

    Prefers the model-provided chat template when available and falls back to a
    generic role-tagged format for tokenizers that do not define one.
    """
    ...
    if _is_deepseek_v32_model(tokenizer, model_name=model_name):
        return _render_deepseek_v32_chat_prompt(
            conversation,
            add_generation_prompt=add_generation_prompt,
        )
    return _render_fallback_chat_prompt(
        conversation,
        add_generation_prompt=add_generation_prompt,
    )
