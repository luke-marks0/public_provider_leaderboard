"""API-based verification using OpenAI-compatible and Tinker backends."""
...
async def _fetch_openai_compatible_logprobs(
    ...
    """Fetch logprobs from an OpenAI-compatible /v1/completions endpoint."""
    extra_body: dict[str, Any] = {"top_logprobs": topk_logprobs}
...
def _to_plain(value: Any) -> Any:
    """Convert OpenAI SDK objects to plain dict/list structures when possible."""
...
def _row_to_position_logprobs(row: Any) -> PositionLogprobs:
    row_plain = _to_plain(row)
    if row_plain is None:
        return None

    if isinstance(row_plain, dict) and "top_logprobs" in row_plain:
        top_raw = _to_plain(row_plain.get("top_logprobs"))
    else:
        top_raw = row_plain
    ...
    if isinstance(top_raw, list):
        for entry in top_raw:
            entry_plain = _to_plain(entry)
            if isinstance(entry_plain, dict):
                token_id = _parse_token_id(entry_plain.get("token_id"))
                if token_id is None:
                    token_id = _parse_token_id(entry_plain.get("token"))
                logprob = entry_plain.get("logprob")
                if token_id is None or not isinstance(logprob, (int, float)):
                    continue
                token_logprobs.append((token_id, float(logprob)))
    elif isinstance(top_raw, dict):
        for token_key, logprob in top_raw.items():
            token_id = _parse_token_id(token_key)
            if token_id is None or not isinstance(logprob, (int, float)):
                continue
            token_logprobs.append((token_id, float(logprob)))
    else:
...
def _openai_logprobs_to_sparse_logprobs(
    logprobs_payload: Any,
    start_idx: int,
    n_tokens: int,
) -> list[PositionLogprobs]:
    """Convert OpenAI-compatible logprobs payload into sparse token-id format.

    Args:
        logprobs_payload: `choice.logprobs` payload from completions API.
    ...
    """
    payload_plain = _to_plain(logprobs_payload)
    if not isinstance(payload_plain, dict):
        raise ValueError("Completion logprobs payload must be an object.")

    if isinstance(payload_plain.get("content"), list):
        rows = payload_plain["content"]
    elif isinstance(payload_plain.get("top_logprobs"), list):
        rows = payload_plain["top_logprobs"]
    else:
        raise ValueError(
            "Completion logprobs payload is missing `content` or `top_logprobs` list."
        )

    slice_rows = rows[start_idx : start_idx + n_tokens]
    ...
    for row in slice_rows:
        result.append(_row_to_position_logprobs(row))

    return result


async def _fetch_all_openai_compatible_logprobs(
    outputs: list[TokenSequence],
    client: AsyncOpenAI,
    model: str,
    topk_logprobs: int,
    verbose: bool,
    backend_label: str = "openai-compatible API",
    request_extra_body: dict[str, Any] | None = None,
    concurrency: int = 10,
) -> list[SparseLogprobs]:
    """Fetch all logprobs from an OpenAI-compatible endpoint concurrently."""
    ...
    if verbose:
        results = []
        desc = f"Verifying via {backend_label}"
...
@torch.inference_mode()
async def verify_outputs_openai_compatible(
    outputs: list[TokenSequence],
    vocab_size: int,
    *,
    temperature: float,
    ...
    """
    Verify LLM outputs using an OpenAI-compatible completions API (async).

    This function takes token sequences (prompt + generated output) and verifies
    whether the outputs could have been produced by the specified model using
    the given sampling parameters. Requests are submitted concurrently.

    In scripts, use: asyncio.run(verify_outputs_openai_compatible(...))
    In Jupyter/async contexts, use: await verify_outputs_openai_compatible(...)
    ...
    Args:
        ...
        top_p: Top-p (nucleus) sampling parameter.
        seed: Random seed used during generation.
        client: An AsyncOpenAI client configured for the verifier backend.
        model: Verification model/deployment identifier.
        topk_logprobs: Number of top logprobs to request. Default: 20.
        verbose: Whether to show progress and print a summary. Default: True.
        backend_label: Label shown in the progress bar.
        request_extra_body: Optional extra body fields forwarded to completions.
    ...
    results = await _fetch_all_openai_compatible_logprobs(
        outputs=outputs,
        client=client,
        model=model,
        topk_logprobs=topk_logprobs,
...
async def verify_outputs_fireworks(
    ...
    *,
    ...
    """Backward-compatible Fireworks wrapper over OpenAI-compatible verification."""
